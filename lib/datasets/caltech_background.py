...
class caltech(imdb):
    ...
    def _load_image_set_index(self):
        ...
        image_index = [png_file.strip() for png_file in png_files if int(png_file[3:5]) in image_set_list]
        return image_index
    ...
    def _load_caltech_annotation(self, index):
        ...
        bboxes = self._annotation[set_num][v_num][frame_num].get(frame_num, None)
        if bboxes is None:
            bboxes = [{'pos':[0,0,640,480]}]
